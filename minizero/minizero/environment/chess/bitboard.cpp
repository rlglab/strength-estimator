...
namespace {
    ...
    std::vector<uint16_t> BuildMoveIndices()
    {
        std::vector<uint16_t> res(4 * 64 * 64);
    ...
    const std::vector<uint16_t> kMoveToIdx = BuildMoveIndices();
...
Move::Move(uint16_t index, bool black)
{
    *this = kIdxToMove[index];
    if (black) {
        this->Mirror();
    }
}
